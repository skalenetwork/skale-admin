...
def test_upsert_schain_record(db):
    with pexec(max_workers=MAX_WORKERS) as executor:
        futures = [
            executor.submit(upsert_schain_record, f'schain-{i}') for i in range(RECORDS_NUMBER)
        ]
...
def test_mark_schain_deleted(db, upsert_db):
    with pexec(max_workers=MAX_WORKERS) as executor:
        futures = [executor.submit(mark_schain_deleted, f'schain-{i}') for i in range(THREADS)]
        for f in futures:
            f.result()
    ...
    assert SChainRecord.select().where(SChainRecord.is_deleted == True).count() == RECORDS_NUMBER  # noqa: E712


def test_schains_first_run(db, upsert_db):
    set_schains_first_run()
    assert SChainRecord.select().where(SChainRecord.first_run == True).count() == RECORDS_NUMBER  # noqa: E712
    # Perform again
    set_schains_first_run()
    assert SChainRecord.select().where(SChainRecord.first_run == True).count() == RECORDS_NUMBER  # noqa: E712

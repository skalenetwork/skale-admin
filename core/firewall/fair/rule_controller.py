...
from .firewall_manager import NFTFairChainFirewallManager
...
class FairController(IRuleController):
    def __init__(
        self,
        controller_name: str,
    ...
    @property
    def firewall_manager(self) -> NFTFairChainFirewallManager:
        if not self._firewall_manager:
            self._firewall_manager = self.create_firewall_manager()
        return self._firewall_manager

    @configured_only
    def create_firewall_manager(self) -> NFTFairChainFirewallManager:
        return NFTFairChainFirewallManager(
            self.name,

...
class SkaledStatus(IStatus):
    ...
    @property
    def exit_time_reached(self) -> bool | None:
    ...
    @property
    def clear_data_dir(self) -> bool | None:
        exit_state = self.exit_state
        if not exit_state:
            return
    ...
    @property
    def start_again(self) -> bool | None:
        exit_state = self.exit_state
        if not exit_state:
            return
    ...
    @property
    def start_from_snapshot(self) -> bool | None:
        exit_state = self.exit_state
        if not exit_state:
            return
...
def init_node_cli_status(schain_name: str) -> NodeCliStatus:
    status_filepath = node_cli_status_filepath(schain_name)
    return NodeCliStatus(status_filepath)


def get_node_cli_status(schain_name: str) -> Optional[NodeCliStatus]:

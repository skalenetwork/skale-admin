...
def is_container_exists(schain_name, container_type=SKALED_CONTAINER, dutils=None):
    dutils = dutils or DockerUtils()
    container_name = get_container_name(container_type, schain_name)
    return dutils.is_container_exists(container_name)


def get_container_image(schain_name, container_type=SKALED_CONTAINER, dutils=None):
    dutils = dutils or DockerUtils()
    container_name = get_container_name(container_type, schain_name)
    return dutils.get_container_image_name(container_name)


def is_container_running(schain_name, container_type=SKALED_CONTAINER, dutils=None):
    dutils = dutils or DockerUtils()
    container_name = get_container_name(container_type, schain_name)
...
def get_container_info(
    image_type: str, schain_name: str, historic_state: bool = False
) -> Tuple[str, str, dict, dict]:
    return (
        get_image_name(image_type=image_type, historic_state=historic_state),
        get_container_name(image_type=image_type, schain_name=schain_name),
        get_container_args(image_type=image_type),
        get_container_custom_args(image_type),
    )
...
def get_ulimits_config(config):
    return list(
        map(
            lambda ulimit: Ulimit(name=ulimit['name'], soft=ulimit['soft'], hard=ulimit['hard']),
            config,
        )
    )
...
def run_container(
    ...
    default_image, container_name, run_args, custom_args = get_container_info(
        image_type, chain_name, historic_state=historic_state
    )

    image_name = image or default_image

    add_config_volume(run_args, chain_name, mode=volume_mode)
    ...
    logger.info(
        arguments_list_string(
            {'Container name': container_name, 'Image name': image_name, 'Args': run_args},
            'Running container...',
        )
    )
    ...
    logger.info(
        arguments_list_string(
            {'Container name': container_name, 'Container id': cont.id},
            'Container created',
            'success',
        )
    )
...
def restart_container(
    ...
    dutils = dutils or DockerUtils()
    container_name = get_container_name(image_type, chain_name)
    logger.info(
...
def is_skaled_container_failed(schain_name: str, dutils: DockerUtils | None = None) -> bool:
    dutils = dutils or DockerUtils()
    name = get_container_name(SKALED_CONTAINER, schain_name)
...
def get_ima_container_time_frame(schain_name: str, dutils: DockerUtils) -> int:
    ...
    ima_time_framing = dutils.get_container_env_value(container_name, 'TIME_FRAMING')
    if ima_time_framing:

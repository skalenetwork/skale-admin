...
def wallet_with_balance(fair):
    address = fair.wallet.address
    fair_balance_wei = fair.web3.eth.get_balance(address)
    return {
        'address': to_checksum_address(address),
        'fair_balance_wei': fair_balance_wei,
        'fair_balance': str(fair.web3.from_wei(fair_balance_wei, 'ether')),
    }
...
def info():
    ...
    res = wallet_with_balance(g.fair)
...
def send_eth():
    ...
    try:
        ...
        send_eth_(g.fair.web3, g.fair.wallet, address, eth_amount)
    except Exception:
